...
export default function DocumentPreviewModal({ open, onClose, documentType, entryId }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => { if (!isOpen) onClose() }}>
      ...
      >
        <DialogHeader>
          <DialogTitle>{documentType === DOCUMENT_TYPES.RESUME ? t('entry.resumePreview') : t('entry.coverLetterPreview')}</DialogTitle>
        </DialogHeader>
          ...
          {loading && (
            <div className="flex items-center justify-center h-64">
              <div className="flex flex-col items-center gap-2 text-muted-foreground">
                ...
                <span className="text-sm">{t('entry.generatingPreview')}</span>
              </div>
            </div>
          )}
