...
export default function CoverLetterForm({ data, onChange }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="font-semibold text-lg text-brand mb-4">{t('coverLetter.addressee')}</h3>
      ...
      <div>
        <h3 className="font-semibold text-lg text-brand mb-4">{t('coverLetter.body')}</h3>
        {paragraphs.map((p, i) => (
        ...
        <Button type="button" variant="outline" size="sm" onClick={addParagraph} className="gap-1"><LuPlus className="w-3 h-3" /> {t('coverLetter.addParagraph')}</Button>
      </div>
      <div>
        <Label>{t('coverLetter.closing')}</Label>
        <Input value={cl.closing || ''} onChange={(e) => set('closing', e.target.value)} placeholder={t('coverLetter.closingPlaceholder')} />
      </div>
    </div>
  )
}
